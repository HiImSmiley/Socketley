...
cli_commands() {
    ...
    echo -e "${BOLD}cluster <dir> [subcommand]${NC}"
    echo "  Multi-daemon cluster inspection (reads shared directory directly)"
    echo -e "  ${CYAN}socketley cluster /cluster ls${NC}        # list daemons"
    echo -e "  ${CYAN}socketley cluster /cluster ps${NC}        # list all runtimes"
    echo -e "  ${CYAN}socketley cluster /cluster group api${NC}  # group members"
    echo -e "  ${CYAN}socketley cluster /cluster stats${NC}     # cluster summary"
    echo -e "  ${CYAN}socketley cluster /cluster watch${NC}     # live refresh"
    echo ""
